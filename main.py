...
app = FastAPI(title="ClientGrowth AI Command Center", version="2.1.0")
...
@app.get("/health")
def health():
    db=db_status(); return {"status":"ok" if db["connected"] else "degraded","service":"clientgrowth-ai-hub","version":"2.1.0","database":db,"integrations":integration_state(),"time":datetime.now(timezone.utc).isoformat()}

@app.get("/api")
def api_root(): return {"status":"ok","service":"clientgrowth-ai-hub","version":"2.1.0","engines":{"database":"PostgreSQL","website_audit":"live_http","maps":"Google Places API","ai":"OpenAI Responses API","qualification":"rules+live provider data"},"endpoints":["/api/health","/api/leads","/api/maps/search","/api/prospect/qualify","/api/whatsapp/check","/api/outreach","/api/campaign/guard"]}
