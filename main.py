...
app = FastAPI(title="ClientGrowth AI Command Center", version="2.0.0")
...
def require_db():
    if not configured():
        raise HTTPException(503, "Persistent storage is not configured. Connect PostgreSQL through Vercel and set DATABASE_URL.")
...
def add_lead(request: Request, name: str=Form(...), lead_type: str=Form(...), location: str=Form(...), signal: str=Form(...), website: str=Form("")):
    require_db()
    lead=create_lead({"name":name.strip(),"type":lead_type.strip(),"location":location.strip(),"signal":signal.strip(),"website":website.strip() or None,"score":0,"source":"manual"})
    add_activity("Lead Radar",f"Added {lead['name']} manually")
    return radar(request)


@app.get("/maps", response_class=HTMLResponse)
def maps(request: Request):
    return templates.TemplateResponse("maps.html", context(request,"maps",results=[]))


@app.post("/maps", response_class=HTMLResponse)
def maps_search(request: Request, query: str=Form(...), location: str=Form("")):
    try:
        results=google_places_search(query,location)
        error=None
    except Exception as exc:
        results=[]; error=str(exc)
    return templates.TemplateResponse("maps.html", context(request,"maps",results=results,query=query,location=location,error=error))
...
def maps_import(request: Request, name: str=Form(...), lead_type: str=Form("Business"), location: str=Form(...), website: str=Form(""), phone: str=Form(""), maps_url: str=Form("")):
    require_db()
    lead=create_lead({"name":name,"type":lead_type,"location":location,"signal":"Imported from Google Places","website":website or maps_url,"score":0,"source":"google-places","metadata":{"phone":phone,"maps_url":maps_url}})
    add_activity("Lead Radar",f"Imported {lead['name']} from Google Places")
    return templates.TemplateResponse("maps.html", context(request,"maps",results=[],notice=f"{lead['name']} was saved to Lead Radar."))


@app.get("/pipeline", response_class=HTMLResponse)
def pipeline(request: Request):
    return templates.TemplateResponse("pipeline.html", context(request,"pipeline",stages=stages))
...
def move_lead(request: Request, lead_id: int, stage: str=Form(...)):
    ...
    add_activity("Opportunity Board",f"Moved {lead['name']} to {stage}")
    return templates.TemplateResponse("pipeline.html", context(request,"pipeline",stages=stages,notice=f"{lead['name']} moved to {stage}."))
...
def outreach(request: Request, company: str="", signal: str="", service: str=""):
    return templates.TemplateResponse("outreach.html", context(request,"outreach",prefill={"company":company,"signal":signal,"service":service}))
...
def automations(request: Request):
    flows=[("Lead discovery","Google Places → qualify → save to database","Live","lead-discovery"),("Website audit","URL → real HTTP fetch → technical/conversion findings","Live","website-audit"),("AI outreach","Prospect signal → OpenAI → review queue","Live","outreach"),("Follow-up","Persistent opportunity → scheduled follow-up","Requires scheduler","follow-up")]
...
def run_automation(request: Request, automation: str=Form(...)):
    if automation=="lead-discovery": target="/maps"
    elif automation=="website-audit": target="/audit"
    elif automation=="outreach": target="/outreach"
    else: target="/settings"
    return templates.TemplateResponse("automations.html", context(request,"automations",flows=[("Lead discovery","Google Places → qualify → save to database","Live","lead-discovery"),("Website audit","URL → real HTTP fetch → technical/conversion findings","Live","website-audit"),("AI outreach","Prospect signal → OpenAI → review queue","Live","outreach"),("Follow-up","Persistent opportunity → scheduled follow-up","Requires scheduler","follow-up")],notice=f"Open {target} to run this workflow with real inputs."))
...
def save_settings_route(request: Request, workspace: str=Form(...), service: str=Form(...)):
    require_db(); save_settings(workspace.strip(),service.strip()); add_activity("Settings","Workspace configuration updated")
    return templates.TemplateResponse("settings.html", context(request,"settings",notice="Workspace settings saved."))


@app.get("/health")
def health():
    db=db_status()
    return {"status":"ok" if db["connected"] else "degraded","service":"clientgrowth-ai-hub","version":"2.0.0","database":db,"integrations":integration_state(),"time":datetime.now(timezone.utc).isoformat()}

@app.get("/api")
def api_root():
    return {"status":"ok","service":"clientgrowth-ai-hub","version":"2.0.0","engines":{"database":"PostgreSQL","website_audit":"live_http","maps":"Google Places","ai":"OpenAI Responses API"},"endpoints":["/api/health","/api/leads","/api/leads/{id}/stage","/api/maps/search","/api/audit","/api/outreach","/api/automations/{automation}"]}
...
@app.get("/api/leads")
def api_leads():
    require_db(); data=list_leads(); return {"status":"ok","count":len(data),"data":data}
...
def api_maps(payload:dict):
    ...
    return {"status":"ok","data":google_places_search(query,location,int(payload.get("max_results",10)))}
...
def api_automation(automation:str):
    if automation not in {"lead-discovery","website-audit","outreach","follow-up"}: raise HTTPException(404,"Automation not found")
    return {"status":"ready","automation":automation,"message":"This workflow requires its live input and provider connection; no simulated job was created."}
