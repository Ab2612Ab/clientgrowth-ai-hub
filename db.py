...
def require_database():
    if not database_url():
        raise RuntimeError("DATABASE_URL is not configured. Connect a PostgreSQL database in Vercel before using persistent features.")
    if psycopg is None:
...
def create_lead(data):
    ensure_schema()
    with connection() as conn:
        row = conn.execute("""INSERT INTO leads(name,type,location,signal,score,stage,website,source,metadata)
            VALUES(%s,%s,%s,%s,%s,'New',%s,%s,%s) RETURNING id,name,type,location,signal,score,stage,website,source,created_at,updated_at""",
            (data["name"], data["type"], data["location"], data["signal"], data.get("score", 0), data.get("website"), data.get("source", "manual"), data.get("metadata", {}))).fetchone()
        cols = ["id","name","type","location","signal","score","stage","website","source","created_at","updated_at"]
...
def add_activity(kind, message):
    ensure_schema()
    with connection() as conn:
        conn.execute("INSERT INTO activities(kind,message) VALUES(%s,%s)", (kind, message))
...
def counts():
    ...
    with connection() as conn:
        ...
        qualified = conn.execute("SELECT COUNT(*) FROM leads WHERE stage IN ('Qualified','Contacted','Replied')").fetchone()[0]
        active = conn.execute("SELECT COUNT(*) FROM leads WHERE stage <> 'New'").fetchone()[0]
        return {"leads": total, "qualified": qualified, "opportunities": active}
...
def db_status():
    try:
        ...
        return {"configured": bool(database_url()), "connected": False, "error": str(exc)}
